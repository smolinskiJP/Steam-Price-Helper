...
system = load_model()
model = system['modelo_elasticnet']
scaler = system['scaler']
dense_features = system['features_densas']
tag_features = system['features_esparsas_tag']
...
tags_selecionadas = st.multiselect(
    "Escolha as tags que melhor descrevem o seu jogo:", 
    ...
    with st.spinner("Analisando o mercado e calculando..."):
        ...
        for tag in tags_selecionadas:
            if tag in tag_features:
                idx = tag_features.index(tag)
                vetor_tags[0, idx] = 1.0 
